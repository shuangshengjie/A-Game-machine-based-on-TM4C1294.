...
{
	char tx[3] = { (reg >> 8) & 0xff, reg & 0xff,  value }; //0d160 = 0b10100000
...
{
	char tx[4] = { (reg >> 8) & 0xff, reg & 0xff,(value >> 8) & 0xff,  value }; //0d160 = 0b10100000
...
{
	char tx[6] = { (reg >> 8) & 0xff, reg & 0xff, (value >> 24) & 0xff, (value >> 16) & 0xff, (value >> 8) & 0xff,  value }; //0d160 = 0b10100000
...
uint16_t VL6180X::readReg16Bit(uint16_t reg)
{
	...
	char tx[2] = { (reg >> 8) & 0xff, reg & 0xff }; //0d160 = 0b10100000
	...
	delay(5);
	stream.requestFrom(SLAVE_ADDRESS, 2);
	data = (uint16_t)stream.read();
	data <<= 8;
...
uint32_t VL6180X::readReg32Bit(uint16_t reg)
{
	...
	delay(5);
	stream.requestFrom(SLAVE_ADDRESS, 4);
	data = (stream.read() << 24);
	data |= (stream.read() << 16);
	data |= (stream.read() << 8);
	data |= stream.read();
